  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      for (let i = 0; i < pts.length; i++) for (let j = i + 1; j < pts.length; j++) {
        ...
        if (d < 120) { g.strokeStyle = `rgba(0,212,255,${0.14 * (1 - d / 120)})`; g.lineWidth = 0.6; g.beginPath(); g.moveTo(a.x, a.y); g.lineTo(b.x, b.y); g.stroke(); }
      }
      pts.forEach((p) => { g.fillStyle = "rgba(0,212,255,.5)"; g.beginPath(); g.arc(p.x, p.y, 1.4, 0, 6.28); g.fill(); });
        ...
        <Reveal>
          <div className="glass relative overflow-hidden">
            ...
            <div className="relative grid items-center gap-11 p-8 md:grid-cols-2 md:p-14">
              <div>
                <div className="mb-5 grid h-15 w-15 place-items-center rounded-2xl border border-line2 text-teal glow-cyan" style={{ background: "linear-gradient(150deg,rgba(0,194,168,.2),rgba(0,212,255,.14))", height: 60, width: 60 }}>
