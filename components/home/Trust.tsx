...
const LOGOS = ["Creality3D", "Everwash", "Yarbo", "Top Amazon Sellers", "DTC Brands", "SaaS Teams"];
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="group relative flex-1 overflow-hidden [mask-image:linear-gradient(90deg,transparent,#000_8%,#000_92%,transparent)]">
          <div className="flex w-max animate-marquee gap-14 group-hover:[animation-play-state:paused]">
            {row.map((l, i) => (
              <span key={i} className="whitespace-nowrap font-grotesk text-xl font-semibold text-[#c8cfe4] opacity-55 transition hover:opacity-100 hover:[text-shadow:0_0_20px_rgba(0,212,255,.6)]">{l}</span>
            ))}
          </div>
        </div>
      </div>
