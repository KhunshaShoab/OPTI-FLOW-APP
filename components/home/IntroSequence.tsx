...
function analyticsTex() {
  ...
  g.strokeStyle = "rgba(0,194,168,0.5)"; g.lineWidth = 2; rr(g, 1, 1, 254, 148, 14); g.stroke();
  g.fillStyle = "rgba(0,194,168,0.9)"; g.beginPath(); g.arc(18, 20, 4, 0, 6.28); g.fill();
  ...
  const v = [40, 66, 52, 84, 70, 96, 78]; v.forEach((h, i) => { g.fillStyle = i % 2 ? "#00C2A8" : "#EC1866"; g.fillRect(22 + i * 32, 140 - h, 18, h); });
      ...
      <div className="pointer-events-none absolute inset-0 grid place-items-center px-6 text-center">
        ...
        <div ref={(el) => (caps.current[1] = el)} className="absolute w-full max-w-4xl opacity-0">
          <div className="grid gap-3 sm:grid-cols-3">
            {[["Amazon Seller", "Where is my order? I need an update.", "#EC1866"], ["Technical Support", "My 3D printer stopped working after the latest firmware update.", "#00C2A8"], ["SaaS Billing", "I've been charged twice. Can you help?", "#FFFFFF"]].map(([t, m, col]) => (
        ...
        <div ref={(el) => (caps.current[5] = el)} className="absolute opacity-0">
          <div className="flex flex-col items-center">
            <span className="relative mb-6 h-20 w-20 animate-spinSlow rounded-[22px]" style={{ background: "conic-gradient(from 200deg,#EC1866,#00C2A8,#FFFFFF,#EC1866)" }}><span className="absolute inset-3 rounded-[16px] bg-[#050816]" /></span>
